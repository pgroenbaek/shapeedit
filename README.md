# shapemod

[![GitHub release (latest by date)](https://img.shields.io/github/v/release/pgroenbaek/shapemod?style=flat&label=Latest%20Version)](https://github.com/pgroenbaek/shapemod/releases)
[![Python 3.6+](https://img.shields.io/badge/Python-3.6%2B-blue?style=flat&logo=python&logoColor=white)](https://www.python.org/)
[![License GNU GPL v3](https://img.shields.io/badge/License-%20%20GNU%20GPL%20v3%20-lightgrey?style=flat&logo=data:image/svg+xml;base64,&logoColor=%23ffffff)](/LICENSE)

This Python module provides safe operations for modifying existing MSTS/ORTS shape files that keeps the shape error-free and usable in MSTS/ORTS.

See also:
- [shapeio](https://github.com/pgroenbaek/shapeio) - provides functions to decode shapes from text files into Python objects and to encode them back into text file format.
- [trackshape-utils](https://github.com/pgroenbaek/trackshape-utils) - offers additional utilities for working with track shapes.

## Installation

### Install from PyPI

```sh
pip install --upgrade shapemod
```

### Install from wheel

If you have downloaded a `.whl` file from the [Releases](https://github.com/pgroenbaek/shapemod/releases) page, install it with:

```sh
pip install path/to/shapemod‑<version>‑py3‑none‑any.whl
```

Replace `<version>` with the actual version number in the filename. For example:

```sh
pip install path/to/shapemod-0.5.0b0-py3-none-any.whl
```

### Install from source

```sh
git clone https://github.com/pgroenbaek/shapemod.git
pip install --upgrade ./shapemod
```

## Usage
TODO

## Running Tests

You can run tests manually or use `tox` to test across multiple Python versions.

### Run Tests Manually
First, install the required dependencies:

```sh
pip install pytest
```

Then, run tests with:

```sh
pytest
```


## Run Tests with `tox`

`tox` allows you to test across multiple Python environments.

### **1. Install `tox`**
```sh
pip install tox
```

### **2. Run Tests**
```sh
tox
```

This will execute tests in all specified Python versions.

### **3. `tox.ini` Configuration**
The `tox.ini` file should be in your project root:

```ini
[tox]
envlist = py36, py37, py38, py39, py310

[testenv]
deps = pytest
commands = pytest
```

Modify `envlist` to match the Python versions you want to support.

## License

This project was created by Peter Grønbæk Andersen and is licensed under [GNU GPL v3](/LICENSE).
